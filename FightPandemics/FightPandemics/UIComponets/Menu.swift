...
class Menu: UIView {
  ...
  var hide = true
    ...
    init(_ parentVC: BaseViewController) {
        ...
        let story = defaults.bool(forKey: "story")
        if story == true {
            self.menuItems.append(contentsOf: [
                                 MenuItem( text: "Analytics Module", addLinkImg: false, onClick: {
                                     if let settingsURL = URL(string: UIApplication.openSettingsURLString) {
                                         UIApplication.shared.open(settingsURL)
                                     }
                                 }),
                                 MenuItem( text: "Network Module", addLinkImg: false, onClick: {
                                     if let parentVC = self.parentVC {
                                         parentVC.performSegue(withIdentifier: "test", sender: parentVC)
                                     }
                                 }),
                                 MenuItem( text: "Terminal Module", addLinkImg: false, onClick: {
         ...
         if MANUAL_STATE_TEST {
                   self.menuItems.append(contentsOf: [
                       MenuItem( text: "Check In", addLinkImg: false, onClick: {
                           if let settingsURL = URL(string: UIApplication.openSettingsURLString) {
                               UIApplication.shared.open(settingsURL)
                           }
                       }),
                       MenuItem( text: "Employee", addLinkImg: false, onClick: {
                           if let parentVC = self.parentVC {
                               parentVC.performSegue(withIdentifier: "test", sender: parentVC)
                           }
                       }),
                       MenuItem( text: "Blog", addLinkImg: true, onClick: {
                           print("Clicked How does this work?") // Dummy function for now
                       }),
                       MenuItem( text: "Events", addLinkImg: true, onClick: {
                           if let url = URL(string: "https://www.covid-watch.org/") {
                               UIApplication.shared.open(url)
                           }
                       }),
                       MenuItem( text: "About Us", addLinkImg: true, onClick: {
                           if let url = URL(string: "https://www.cdc.gov/coronavirus/2019-nCoV/index.html") {
                               UIApplication.shared.open(url)
                           }
                       }),
                       MenuItem(text: "Donate", addLinkImg: true, onClick: {
